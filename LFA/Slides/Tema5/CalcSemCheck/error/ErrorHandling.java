...
public class ErrorHandling
{
   ...
   /**
    * Define current source code file path.
    *
    * <p><dl><dt><b>Precondition:</b></dt>
    *    <dd>{@code filePath != null && filePath.length() > 0}</dd>
    * </dl></p>
    *
   ...
   public static void setFilePath(String filePath)
   {
      ...
      ErrorHandling.filePath = filePath;
   }

   /**
    * Current file path.
    *
    * @return {@code String} current file path (null if none defined).
    */
   ...
   {
      logFile.printf("[%s%s%s%s] %s\n", prefixFormat[type-1], prefixMsg[type-1], RESET, filePath == null ? "" : ", "+filePath,text);
      logFile.flush();
   }

   protected static void printMessage(int line, String text, int type)
   {
      logFile.printf("[%s%s%s%s at line %d] %s\n", prefixFormat[type-1], prefixMsg[type-1], RESET, filePath == null ? "" : ", "+filePath, line, text);
      logFile.flush();
   }
