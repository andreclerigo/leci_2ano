...
Graph* GraphCopy(const Graph* g) {
  ...
  List* vList = g->verticesList;
  int size = ListGetSize(vList);
  for (ListMoveToHead(vList); ListGetCurrentPos(vList) < size; ListMoveToNext(vList))
  {
    struct _Vertex* v = (struct _Vertex*)ListGetCurrentItem(vList);
    List* eList = v->edgesList;
    int sizeEdges = ListGetSize(eList);
    for (ListMoveToHead(eList); ListGetCurrentPos(eList) < sizeEdges; ListMoveToNext(eList)) {
      struct _Edge* e = (struct _Edge*)ListGetCurrentItem(eList);
...
Graph* GraphFromFile(FILE* f) {
  ...
  fscanf(f, "%d%d%d%d", &isDigraph, &isWeighted, &numVertices, &numEdges);
  ...
  if (isWeighted)
  {
    ...
    {
      fscanf(f, "%d%d%f", &v1, &v2, &weight);
      if (v1 != v2)
        GraphAddWeightedEdge(g, v1, v2, (int)(weight * 100));
    }
  } else {
    for (int i = 0; i < numEdges; i++)
    {
...
unsigned short GraphRemoveEdge(Graph* g, unsigned int v, unsigned int w) {
  ...
  //Dummy vertex para poder procurar na lista
  struct _Vertex* vertexDummy = (struct _Vertex*) malloc(sizeof(struct _Vertex));
  //Dummy edge para poder procurar na lista
  struct _Edge* edgeDummy = (struct _Edge*) malloc(sizeof(struct _Edge));
  List* vList = g->verticesList;

  if (g->isDigraph == 1)
  {
    vertexDummy->id = v;
    edgeDummy->adjVertex = w;

    ListSearch(vList, vertexDummy);
    struct _Vertex* vertexV = (struct _Vertex*)ListGetCurrentItem(vList);
    List* eListV = vertexV->edgesList;

    if (ListSearch(eListV, edgeDummy) == -1)
    {
      free(vertexDummy);
      free(edgeDummy);
      return 0;
    ...
    ListRemoveCurrent(eListV);

    vertexDummy->id = w;
    ListSearch(vList, vertexDummy);
    struct _Vertex* vertexW = (struct _Vertex*)ListGetCurrentItem(vList);
    ...
    ListSearch(vList, vertexDummy);
    struct _Vertex* vertexW = (struct _Vertex*)ListGetCurrentItem(vList);
    List* eListW = vertexW->edgesList;
    edgeDummy->adjVertex = v;

    if (ListSearch(eListW, edgeDummy) == -1)
    {
      free(vertexDummy);
      free(edgeDummy);
      return 0;
    }
    ListRemoveCurrent(eListW);
    vertexW->outDegree--;
    g->numEdges--;
  }
...
unsigned short GraphCheckInvariants(const Graph* g) {
  ...
  if (g->isComplete && g->isDigraph)
    assert(g->numEdges == (g->numVertices)^2 - g->numVertices);
